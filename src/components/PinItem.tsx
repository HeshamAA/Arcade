  ...
  return (
    <div className="flex items-start gap-7 opacity-40" id={`pin-${numberOfPin}`}>
      <div className="flex flex-col items-center">
        <div>{numberOfPin}</div>
        <div
          className={`h-[60px] w-2 rounded bg-[#cccccc]  outline-0  border-[black] hidden pin-${numberOfPin}-parent`}
      ...
      <div className="flex flex-col max-w-[500px]">
        ...
        <div className={`pin-${numberOfPin}-parent hidden font-robert-regular`}>{text}</div>
      </div>
    </div>
  );
};
